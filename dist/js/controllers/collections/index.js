...
exports.addAppointment = exports.addTimeSlot = exports.getTimeSlots = exports.getAppointments = exports.getDoctors = void 0;
    ...
    try {
        ...
        if (body) {
            if ([null, undefined, ''].includes(body.fromtime)) {
                ...
                const fromCheck = yield appointments_1.default.find(condition).limit(1);
                const toCheck = yield appointments_1.default.find(condition1);
                console.log('fromCheck', fromCheck);
                console.log(toCheck);
                if (fromCheck.length === 0 && toCheck.length === 0) {
                    // Appointment.find().count;
                    const latestRecord = yield appointments_1.default.find().sort({ '_id': -1 }).limit(1);
                    let id = 1;
                    if (latestRecord) {
                        id = Number(latestRecord[0]._id) + 1;
                    }
                    const appoinment = new appointments_1.default({
                        _id: id,
                        fromtime: body.fromtime,
                        totime: body.totime,
                        status: body.status,
                    ...
                    const newAppoinment = yield appoinment.save();
                    const allAppointment = yield appointments_1.default.find();
